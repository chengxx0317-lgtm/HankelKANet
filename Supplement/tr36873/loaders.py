...
def compute_Einc_and_rmap(tx_mask, f=5.9e9, dx=1.0, normalize=True):
    ...
    r = np.sqrt((xx - x0) ** 2 + (yy - y0) ** 2) * dx + 1e-8  # 物理距离 (m)
    r_map = r.astype(np.float32)
    if normalize:
        eps = 1e-9
        r_map = (r_map - r_map.min()) / (r_map.max() - r_map.min() + eps)
    # 入射场
    '''H0 = sp.hankel2(0, k * r)
    Einc = -1j / 4 * H0
    ...
    return r_map#Einc_real, Einc_imag, Einc_real_raw, Einc_imag_raw,


class Dataset_RadioMapSeer(Dataset):
    def __init__(self, phase="train",
                 dir_dataset='D:/PycharmProjects/PythonProjects/CXX/PEFNet/data/RadioMapSeer/',
                 numTx=80,
                 img_size=256,
                 f=5.9e9,
                 dx=1.0,
