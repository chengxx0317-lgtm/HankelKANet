...
MODEL_PATH = "./knn_baseline_run/knn_baseline_best.joblib"
SAVE_DIR = "./knn_baseline_inference"
...
def batched_predict(model, feats: np.ndarray, batch_size: int) -> np.ndarray:
    preds = []
    n = feats.shape[0]
    for s in range(0, n, batch_size):
        e = min(s + batch_size, n)
        preds.append(model.predict(feats[s:e]).astype(np.float32))
...
def main():
    ...
    for global_idx, (map_ind, tx_idx) in enumerate(test_refs):
        ...
        pred_norm = batched_predict(model, feats, PRED_BATCH).reshape(IMG_SIZE, IMG_SIZE)
