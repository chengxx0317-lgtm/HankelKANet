...
RUN_DIR = "./knn_baseline_run"
MODEL_PATH = os.path.join(RUN_DIR, "knn_baseline_best.joblib")
...
PIXELS_PER_REF_TRAIN = 16
PIXELS_PER_REF_VAL = 16

N_NEIGHBORS_CANDIDATES = [3, 5, 10, 20]
...
def main():
    ...
    total_trials = len(N_NEIGHBORS_CANDIDATES) * len(WEIGHTS_CANDIDATES) * len(P_CANDIDATES)
    trial_id = 0
    ...
    for n_neighbors, weights, p in product(
        ...
        trial_id += 1
        print(f"[{trial_id}/{total_trials}] Training KNN with "
              f"n_neighbors={n_neighbors}, weights={weights}, p={p}")
