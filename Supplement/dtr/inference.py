...
# ============================================================
# DTR baseline inference / evaluation
# ------------------------------------------------------------
# Aligns with your final inference.py:
# - same test split
# - same fixed visualization indices
# - same metrics: MSE / MAE / MAE(dB) / RMSE(dB) / R2
# - saves paper-ready 3-panel figures
# ============================================================
...
MODEL_PATH = "./dtr_baseline_run/dtr_baseline_best.joblib"
SAVE_DIR = "./dtr_baseline_inference"
...
def main():
    ...
    for global_idx, (map_ind, tx_idx) in enumerate(test_refs):
        ...
        pred_norm = model.predict(feats).reshape(IMG_SIZE, IMG_SIZE).astype(np.float32)
        ...
        if SAVE_FULL_OUTPUT:
            preds_all.append(pred_norm)
            labels_all.append(label_norm)
    ...
    print("====== DTR Baseline Test Results ======")
