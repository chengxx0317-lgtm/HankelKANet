...
IMG_SIZE = 256
EVAL_PHASE = "test"
SAVE_DIR = "./baseline_tr38901_mapaware_results"
...
USR_H = 1.5
BS_H = 10.0
SCENARIO = "umi"   # consistent with your dataset scale and prior prototype

# Reproducibility
SEED = 2025
STOCHASTIC = False  # main-table recommendation: deterministic expected loss
...
def calculate_tr38901_path_loss_map(
    h: int,
    w: int,
    tx_row: int,
    tx_col: int,
    ...
    if scenario == "umi":
        # LOS probability consistent with the uploaded prototype
        p_los = np.where(
            d2d <= 18.0,
            1.0,
            18.0 / d2d_safe + np.exp(-d2d_safe / 36.0) * (1.0 - 18.0 / d2d_safe),
        )
        # Coefficients from the user's prototype
        a1, b1 = 32.4, 21.0
        a2, b2, c2 = 32.4, 40.0, 9.5
        p1, q1, r1, s1 = 22.4, 35.3, 21.3, 0.3
        sigma_los, sigma_nlos = 4.0, 7.82
        hE = 1.0
    elif scenario == "uma":
        # Also kept for completeness
        if usr_h <= 13:
            cht = 0.0
        else:
            cht = ((usr_h - 13.0) / 10.0) ** 1.5
        p_los = np.where(
            d2d <= 18.0,
            1.0,
            (18.0 / d2d_safe + np.exp(-d2d_safe / 63.0) * (1.0 - 18.0 / d2d_safe))
            * (1.0 + cht * 1.25 * (d2d_safe / 100.0) ** 3 * np.exp(-d2d_safe / 150.0)),
        )
        a1, b1 = 28.0, 22.0
        a2, b2, c2 = 28.0, 40.0, 9.0
        p1, q1, r1, s1 = 13.54, 39.08, 20.0, 0.6
        sigma_los, sigma_nlos = 4.0, 6.0

        gg = np.where(d2d <= 18.0, 0.0, 1.25 * (d2d_safe / 100.0) ** 3 * np.exp(-d2d_safe / 150.0))
        cc = np.where(usr_h < 13.0, 0.0, gg * ((usr_h - 13.0) / 10.0) ** 1.5)
        prob1 = 1.0 / (1.0 + cc)
        # expectation-friendly effective hE
        hE = prob1 * 1.0 + (1.0 - prob1) * max(usr_h - 1.5, 1.0)
    else:
        raise ValueError("scenario must be 'umi' or 'uma'")

    p_los = np.clip(p_los, 0.0, 1.0)

    d_bp = 4.0 * (bs_h - hE) * (usr_h - hE) * fc / c
    fc_ghz = fc * 1e-9

    pl1 = a1 + b1 * np.log10(d3d) + 20.0 * np.log10(fc_ghz)
    pl2 = (
        a2
        + b2 * np.log10(d3d)
        + 20.0 * np.log10(fc_ghz)
        - c2 * np.log10(d_bp ** 2 + (bs_h - usr_h) ** 2)
    )

    pl_los = np.where(d2d <= d_bp, pl1, pl2)
    pl_nlos = p1 + q1 * np.log10(d3d) + r1 * np.log10(fc_ghz) - s1 * (usr_h - 1.5)
    pl_nlos_eff = np.maximum(pl_los, pl_nlos)

    if stochastic:
        has_los = rng.binomial(1, p_los, size=p_los.shape)
        noise_los = rng.normal(loc=0.0, scale=sigma_los, size=p_los.shape)
        noise_nlos = rng.normal(loc=0.0, scale=sigma_nlos, size=p_los.shape)
        pl = np.where(
            has_los == 1,
            pl_los + noise_los,
            pl_nlos_eff + noise_nlos,
        )
    else:
        pl = p_los * pl_los + (1.0 - p_los) * pl_nlos_eff
...
def main():
    ...
    for global_idx in tqdm(range(len(test_set)), desc="TR38.901 map-aware baseline"):
        ...
        pl_pos_db = calculate_tr38901_path_loss_map(
            h=IMG_SIZE,
            w=IMG_SIZE,
            tx_row=tx_row,
            tx_col=tx_col,
    ...
    print("====== TR 38.901 Map-Aware Baseline Test Results ======")
