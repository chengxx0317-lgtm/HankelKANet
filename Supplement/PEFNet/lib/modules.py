...
class PhysicsOperator(nn.Module):
    ...
    def build_kernel(self, size):
        ...
        cy, cx = h // 2, w // 2# 核中心坐标（卷积核通常以中心对称）
        r = torch.sqrt((xx - cx) ** 2 + (yy - cy) ** 2) * self.dx + 1e-6# 实际距离（米）
        # 使用平滑近似的Hankel核：H0^(2)(kr) ≈ exp(-jkr)/sqrt(r)
...
class PhysicsInformedModel(nn.Module):
    ...
    def forward(self, inputs, E_inc_raw, chi, label=None):
        ...
        loss_total = self.lambda_d * loss_data + 0.1*loss_phy
        return {
            "E_pred": E_pred,
            "PL_init": PL_init,
            "PL_pred": PL_pred,
